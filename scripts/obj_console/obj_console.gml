...
#macro	CSL_DRAW_CURSOR			0x10000000
#macro	CSL_FIRST_BACKSPACE		0x20000000
#macro	CSL_FIRST_CURSOR_MOVE	0x40000000
#macro	CSL_CONSOLE_ACTIVE		0x80000000

// Condenses the code required to check any of the Console's bit flags' current states into macro values.
#macro	CAN_DRAW_CURSOR			(stateFlags & CSL_DRAW_CURSOR)
#macro	IS_FIRST_BACKSPACE		(stateFlags & CSL_FIRST_BACKSPACE)
#macro	IS_FIRST_CURSOR_MOVE	(stateFlags & CSL_FIRST_CURSOR_MOVE)
#macro	IS_CONSOLE_ACTIVE		(stateFlags & CSL_CONSOLE_ACTIVE)
...
function obj_console(_index) : base_struct(_index) constructor{
	// 
	stateFlags = CSL_FIRST_BACKSPACE | CSL_FIRST_CURSOR_MOVE | CSL_DRAW_CURSOR;
	...
	end_step = function(){
		...
		if (cursorTimer >= CURSOR_FLASH_INTERVAL){
			if (CAN_DRAW_CURSOR) {stateFlags &= ~CSL_DRAW_CURSOR;}
			else				 {stateFlags |= CSL_DRAW_CURSOR;}
			cursorTimer = 0.0;
		}
		...
		if (keyboard_check(vk_backspace) && commandCursorPos > 1){
			...
			if (backspaceTimer <= 0.0){
				...
				if (IS_FIRST_BACKSPACE){ // First backspace; longer duration before next character is deleted. 
					stateFlags	   &= ~CSL_FIRST_BACKSPACE;
		...
		if (_movement != 0){
			...
			if (cursorMoveTimer <= 0.0){
				...
				if (IS_FIRST_CURSOR_MOVE){ // Like the backspace code; the first movement will always have a longer duration until the next movement.
					stateFlags	   &= ~CSL_FIRST_CURSOR_MOVE;
	...
	enable_console_window = function(){
		...
		// 
		stateFlags		   |= CSL_CONSOLE_ACTIVE | CSL_FIRST_BACKSPACE | 
							  CSL_FIRST_CURSOR_MOVE | CSL_DRAW_CURSOR;
	...
	/// game's performance, camera position, room info, and so on. An optional argument can be provided to
	/// customize what sections of the debug window are visible upon its initialization.
	/// @param {String}	flags	Determines what will be displayed on the debug window when opened
	cmd_toggle_debug_info = function(_flags = "1|2|3"){
		...
		with(DEBUGGER){ // Determine if the state bit to flip to 0 or 1 depending on what its current value is.
			...
			if (_windowVisible) {stateFlags &= ~DBG_SHOW_WINDOW;}
			else				{stateFlags |=  DBG_SHOW_WINDOW;}
			
			// 
			var _flagsTrimmed	= string_trim(_flags);
			var _length			= string_length(_flagsTrimmed);
			var _curChar		= "";
			for (var i = 1; i <= _length; i++){
				_curChar = ord(string_char_at(_flagsTrimmed, i));
				if (_curChar == 0x7C) // Skip if "|" is the current character. 
					continue;
					
				if (_curChar >= vk_0 && _curChar <= vk_9){ // Numerical symbols.
					stateFlags |= (1 << (_curChar - vk_0));
					continue;
				}
				
				if (_curChar >= vk_a && _curChar <= vk_z){ // Uppercase characters from "A" to "Z".
					stateFlags |= (1 << (_curChar - vk_a));
					continue;
				}
				
				if (_curChar >= 0x61 && _curChar <= 0x7A) // Factors in lowercase characters.
					stateFlags |= (1 << (_curChar - 0x61));
			}
		}
	...
	cmd_debug_show_playtime = function(){
		with(DEBUGGER){
			if (CAN_SHOW_TIME)	{stateFlags &= ~DBG_SHOW_TIME;}
			else				{stateFlags |=  DBG_SHOW_TIME;}
		}
	}
	...
	cmd_debug_show_game_state = function(){
		with(DEBUGGER){
			if (CAN_SHOW_GAME_STATE) {stateFlags &= ~DBG_SHOW_GAME_STATE;}
			else					 {stateFlags |=  DBG_SHOW_GAME_STATE;}
		}
	}
	...
	cmd_debug_show_fps = function(){
		with(DEBUGGER){
			if (CAN_SHOW_FPS)	{stateFlags &= ~DBG_SHOW_FPS;}
			else				{stateFlags |=  DBG_SHOW_FPS;}
		}
	}
	...
	cmd_debug_show_render_data = function(){
		with(DEBUGGER){
			if (CAN_SHOW_RENDER) {stateFlags &= ~DBG_SHOW_RENDER;}
			else				 {stateFlags |=  DBG_SHOW_RENDER;}
		}
	}
	...
	cmd_debug_show_room_data = function(){
		with(DEBUGGER){
			if (CAN_SHOW_ROOM)	{stateFlags &= ~DBG_SHOW_ROOM;}
			else				{stateFlags |=  DBG_SHOW_ROOM;}
		}
	}
	...
	cmd_debug_show_camera_data = function(){
		with(DEBUGGER){
			if (CAN_SHOW_CAMPOS) {stateFlags &= ~DBG_SHOW_CAMPOS;}
			else				 {stateFlags |=  DBG_SHOW_CAMPOS;}
		}
	}
	...
	cmd_debug_show_instance_data = function(){
		with(DEBUGGER){
			if (CAN_SHOW_INSTANCES) {stateFlags &= ~DBG_SHOW_INSTANCES;}
			else					{stateFlags |=  DBG_SHOW_INSTANCES;}
		}
	}
	...
	cmd_debug_show_map_data = function(){
		with(DEBUGGER){
			if (CAN_SHOW_MAP_INFO)	{stateFlags &= ~DBG_SHOW_MAP;}
			else					{stateFlags |=  DBG_SHOW_MAP;}
		}
	}
