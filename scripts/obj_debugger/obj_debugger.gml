...
function obj_debugger(_index) : base_struct(_index) constructor{
	// 
	stateFlags = (1 << DBG_SHOW_TIME) | (1 << DBG_SHOW_GAME_STATE) | (1 << DBG_SHOW_FPS);
	...
	/// this struct.
	cleanup = function(){
		if (surface_exists(surfInfoWindow)) {surface_free(surfInfoWindow);}
	...
	end_step = function(){
		...
		if (keyboard_check_pressed(vk_f1)){
			if (!CAN_SHOW_COLLIDERS) {stateFlags |= (1 << DBG_SHOW_COLLIDERS);}
			else					 {stateFlags &= ~(1 << DBG_SHOW_COLLIDERS);}
		}
		...
		if (!CAN_SHOW_WINDOW){
			if (keyboard_check_pressed(vk_d)) {stateFlags |= (1 << DBG_SHOW_WINDOW);}
			return;
		}
	...
	draw_game_state_info = function(_yOffset){
		if (!CAN_SHOW_GAME_STATE) {return 0;}
	...
	draw_instance_info = function(_yOffset){
		if (!CAN_SHOW_INSTANCES) {return 0;}
