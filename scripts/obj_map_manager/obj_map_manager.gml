...
function obj_map_manager(_index) : base_struct(_index) constructor{
	// 
	stateFlags = (1 << PLAYER_VISIBLE);
	
	// 
	mapData		= -1;
	mapColor	= HEX_DARK_ORANGE;
	...
	initialize_map_cell = function(_x, _y, _borderIndex, _iconIndex = -1, _doorData = []){
		// 
	...
	/// @param {Real}	cellY
	/// @param {Real}	color
	draw_map_cell = function(_x, _y, _cellX, _cellY, _color){
		// 
		...
		with(cells[# _cellX, _cellY]){
			...
			// 
			draw_sprite_ext(spr_rectangle, 0, _x, _y, MAP_TILE_WIDTH, MAP_TILE_HEIGHT, 0, _color, 1.0);
